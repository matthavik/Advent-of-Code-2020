...
seats = [int(seat.replace("F", "0").replace("B", "1").replace("L", "0").replace("R", "1"), 2)for seat in open("input.txt").readlines()]
print(f"Highest seat id: {max(seats)}")

your_seat = [x for x in range(min(seats), max(seats)) if x not in seats].pop()
